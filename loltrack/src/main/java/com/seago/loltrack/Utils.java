...
class Utils {
	...
	// Filters out a championValueMap array by a specified role
	public static ArrayList<ChampionValueMap> filterByRole(ArrayList<ChampionValueMap> championValueMap, int roleId) {
		// The array to store champions that fulfill the specified role
		ArrayList<ChampionValueMap> result = new ArrayList<ChampionValueMap>();

		// Go through all the champions in the list
        for (ChampionValueMap aChampionValueMap : championValueMap) {
            // Get the champions roles
            String championName = aChampionValueMap.get("name");
	...
	// Methods for retrieving a certain statistic from a player
    private class stats {
